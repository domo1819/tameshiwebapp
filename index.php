		...
		<main>
			...
			<div class="engine2">
				...
				<?php
					...
					$result = pg_query($conn, "SELECT warn_info.id, warn_info.user_id, warn_info.timestamp, warn_info.punish_id,  warn_info.is_payment, region_data.id, region_data.region_name, car_data.id, car_data.car_region_id, car_data.car_classify_num, car_data.car_classify_hiragana, car_data.car_number FROM warn_info,region_data WHERE region_name LIKE '%{$_POST['word']}%' OR car_number LIKE '%{$_POST['word']}%'"); 
					...
					echo "<table border=1><tr><th>ID</th><th>地方</th><th>車（地方）</th><th>分類番号</th><th>分類ひらがな</th><th>ナンバー</th><th>userID</th><th>日時</th><th>刑罰ID</th><th>支払い状況</th></tr>";
					//データの出力
					foreach($arr as $rows){
						echo "<tr>\n";
