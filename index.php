		...
		<main>
			...
			<div class="engine2">
				...
				<?php
					...
					$result = pg_query($conn, "SELECT region_data.id, region_data.region_name, car_data.id, car_data.car_region_id, car_data.car_classify_num, car_data.car_classify_hiragana, car_data.car_number, warn_info.id, warn_info.user_id, warn_info.timestamp, warn_info.car_data_id, warn_info.punish_id, warn_info.is_payment FROM region_data, car_data, warn_info WHERE region_name LIKE '%{$_POST['word']}%' OR car_number LIKE '%{$_POST['word']}%' OR user_id LIKE '%{$_POST['word']}%'"); 
