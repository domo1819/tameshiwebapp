		...
		<main>
			...
			<div class="engine2">
				...
				<?php
					...
					$result = pg_query($conn, "SELECT warn_info.id, warn_info.user_id, warn_info.timestamp, warn_info.punish_id, warn_info.is_payment, warn_info.car_data_id, car_data.id, car_data.car_region_id, car_data.car_classify_num, car_data.car_classify_hiragana, car_data.car_number, region_data.id, region_data.region_name FROM warn_info, car_data, region_data WHERE region_name LIKE '%{$_POST['word']}%' OR car_number LIKE '%{$_POST['word']}%'"); 
					...
					echo "<table border=1><tr><th>ID</th><th>userID</th><th>日時</th><th>刑罰ID</th><th>支払い状況</th><th>地方</th><th>地方（車）</th><th>分類番号</th><th>分類ひらがな</th><th>ナンバー</th></tr>";
					//データの出力
					foreach($arr as $rows){
						echo "<tr>\n";
